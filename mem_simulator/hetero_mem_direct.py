...
class HeterogeneousMemorySystem:
    def __init__(self, cxl_layout_file, trace_dir):
        ...
        if (args.dram_size is not None):
            ...
            self.dram_capacity = self.cxl_capacity // self.capacity_ratio
        else:
            self.capacity_ratio = 8
            self.dram_capacity = self.cxl_capacity // self.capacity_ratio

        self.dram_set_count = self.dram_capacity // 1  # 4-way set associative
        self.dram_cache = np.full((self.dram_set_count), -1, dtype=int)  # -1 表示空位
        #self.dram_lru = np.zeros((self.dram_set_count), dtype=int)  # 记录 LRU 顺序
    ...
    # 处理cache hit的函数
    def cache_hit(self, page_number):
        set_index = self.find_dram_set(page_number)

        # 命中：更新 LRU，将命中的页号移动到末尾
    ...
    def cache_miss(self, page_number):
        set_index = self.find_dram_set(page_number)
        self.dram_cache[set_index] = page_number
        replace = True
    ...
    def is_hit(self, page_number):
        set_index = self.find_dram_set(page_number)
        hit = (page_number == self.dram_cache[set_index])
        return hit
    ...
    def access_dram_cache(self, page_number):
        ...
        if self.is_hit(page_number):
            self.cache_hit(page_number)
            return True, False
        else:
            replace = self.cache_miss(page_number)
    ...
    def simulate_access(self):
        ...
        output_filename = f'direct_lru_{self.dram_capacity * 4 // 1024}MB.csv'
