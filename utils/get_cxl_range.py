...
parser.add_argument('--cacheblock', choices=['2M'], default='2M', help='The size of DRAM cacheblock')
...
if (args.cacheblock == '4096' or args.cacheblock == '4K'):
    ...
    roi_dir = f'roi_{args.cacheblock}'

output_prefix = ""
trace_dir = f"/home/yxr/downloads/test_trace/global_dist/{roi_dir}/" + args.benchname + "/" + str(args.period)

if (args.type == 'v'):
    output_dir=f"/home/yxr/downloads/test_trace/res/{roi_dir}/" + args.benchname + "/" + str(args.period) + "/Zipfan_Hot_Dist/VPN/"
    trace_suffix = 'vout'
elif (args.type == 'p'):
    output_dir=f"/home/yxr/downloads/test_trace/res/{roi_dir}/" + args.benchname + "/" + str(args.period) + "/Zipfan_Hot_Dist/PPN/"
    trace_suffix = 'pout'
...
for filename in os.listdir(trace_dir):
    if (filename.endswith(trace_suffix)):
        ...
        data = get_trace_data(filename)
        pn_list = [entry[0] for entry in data]
        cur_pn_set = set(pn_list)
        distinct_pn = distinct_pn.union(cur_pn_set)
    
print(f"size: {len(distinct_pn) * 2}MB")