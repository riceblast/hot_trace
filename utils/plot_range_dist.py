...
parser.add_argument('-b', '--begin_numbers', required=True, type=int, help="First of file index to process(inclusive)")
parser.add_argument('-n', '--numbers', required=True, type=int, help="Last of file index to process(inclusive)")
...
for n in range(args.begin_numbers, args.numbers + 1):
    file_path = os.path.join(args.input_dir, f"{args.file_prefix}_{n}.hot_dist.pout")
...
tick_positions = range(args.begin_numbers, args.numbers + 1)
...
save_path = os.path.join(args.output_dir, f"{args.file_prefix}_hot_dist_over_time_{args.begin_numbers}_{frequency_suffix}.png")
